...
#include <algorithm>
    ...
    #define llama_mmap_advise madvise
    #define LLAMA_MMAP_ADVISE_WILLNEED MADV_WILLNEED
    #define LLAMA_MMAP_ADVISE_RANDOM MADV_RANDOM
#else
    #define llama_mmap_advise posix_madvise
    #define LLAMA_MMAP_ADVISE_WILLNEED POSIX_MADV_WILLNEED
    #define LLAMA_MMAP_ADVISE_RANDOM POSIX_MADV_RANDOM
#endif
...
struct llama_mmap::impl {
    ...
    impl(struct llama_file * file, size_t prefetch, bool numa, const llama_mmap::ranges & lazy_ranges) {
        ...
        auto advise = [&](size_t beg, size_t end, int advice, const char * name) {
            ...
            if (llama_mmap_advise((char *) addr + beg, end - beg, advice)) {
                LLAMA_LOG_WARN("warning: mmap advice (.., %s) failed: %s\n", name, strerror(errno));
            }
        };

        if (prefetch > 0) {
            for (const auto & range : ranges_complement(lazy_ranges, std::min(file->size(), prefetch))) {
                advise(range.first, range.second, LLAMA_MMAP_ADVISE_WILLNEED, "MADV_WILLNEED");
            }
        }
        for (const auto & range : lazy_ranges) {
            advise(range.first, range.second, LLAMA_MMAP_ADVISE_RANDOM, "MADV_RANDOM");
        }
        if (numa) {
            if (llama_mmap_advise(addr, file->size(), LLAMA_MMAP_ADVISE_RANDOM)) {
                LLAMA_LOG_WARN("warning: mmap advice (.., MADV_RANDOM) failed: %s\n",
                        strerror(errno));
            }
        }
